...
%ref is chosen to be the adjusted hub
...
ft_write_cifti_mod(['!!Your Path Here!!/Spotlight/AdjHubsGroupNetworks/Hub',char(num2str(tarhubs(thehub))),'MSC',char(subs(thesubs)),'_trim.dtseries.nii'],template);
end

%Log needed info%
save(['!!Your Path Here!!/Spotlight/AdjHubsGroupNetworks/Hub',char(num2str(tarhubs(thehub))),'Minlog.mat'],'winneridxlog','MinDistIndex','WinnerDistCell');
...
imagesc([newhubmats{thehub,1},WashUperHubNetCorrProfile(:,thehub)]);
    ...
    colorbar
    colormap jet
    saveas(1,['!!Your Path Here!!/Spotlight/AdjHubsGroupNetworks/Hub ',char(num2str(tarhubs(thehub))),' Ref:WashU120 Network Corrs By Group Net.jpg']);
    save('!!Your Path Here!!/Spotlight/AdjHubsGroupNetworks/HubLoopTest.mat','newhubmats','MinDistIndex','MapMat','FiltMapMat','badpointtrack','winneridxlog','winnerActidxlog','WashUperHubNetCorrProfile');
end
