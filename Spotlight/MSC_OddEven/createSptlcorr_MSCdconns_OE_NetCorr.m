function createSptlcorr_MSCdconns_OE_NetCorr(target,TarName,cortexOnly,outputdir,dconnData, outputname)
...
if cortexOnly==1
    target = target(1:59412,1); %DMS changed since the target is now a 1D vector
    sizedata = size(target,1);
    
    % Load template variants file
     template = ft_read_cifti_mod('/!!Your Path Here!!/templates/MSC01_allses_mean_native_freesurf_vs_120sub_corr.dtseries.nii');
    template.data = [];
...
disp(sprintf('Template Loaded: %s', datestr(now)));
            ...
            for i=1:sizedata
                template.data(i,1) = paircorr_mod(tar_corrmap(:,1),cifti_corrmap(:,i)); %DMS Target always the same vector%
            end
            clear cifti_corrmap
