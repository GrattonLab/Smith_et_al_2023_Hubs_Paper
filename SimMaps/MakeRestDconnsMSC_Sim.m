%This script makes the similarity maps aka spatial correlation maps of the
...
outdir = '!!Your Path Here!!/SimMaps/Good_CG_CorrMaps'; %output dir currently not used but can be worked in the script
...
for i=1:numel(subs)
    ...
    if CortexOnly == 1
        ...
        disp(sprintf('Reading %s: %s', '!!Your Cortex Plus Template dtseries.nii File!!', datestr(now)));
    ...
    createSptlcorr_MSCdconns('/!!Your Path Here!!/Atlases', '120_allsubs_corr',1,'/!!Your Output dir!!',template.data, outputfile);
